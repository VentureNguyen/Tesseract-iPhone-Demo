...
static inline int DecimalDigitValue(int ch)
{
  if (ch >= '0' && ch <= '9') {
    return ch-'0';
  } else {
...
double streamtofloat(FILE* s)
{
  ...
  // Actual number parsing
  for (; (c != EOF) && (d = DecimalDigitValue(c)) >= 0; c = fgetc(s))
    v = v*10 + d;
  if (c == '.') {
    for (c = fgetc(s); (c != EOF) && (d = DecimalDigitValue(c)) >= 0; c = fgetc(s)) {
      w = w*10 + d;
      k *= 10;
    }
  }
...
double strtofloat(const char* s)
{
  ...
  // Actual number parsing
  for (; *s && (d = DecimalDigitValue(*s)) >= 0; s++)
    v = v*10 + d;
  if (*s == '.') {
    for (++s; *s && (d = DecimalDigitValue(*s)) >= 0; s++) {
      w = w*10 + d;
      k *= 10;
    }
  }
